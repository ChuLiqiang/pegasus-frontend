...
{
    return m_filterPatternRaw;
}

void QQmlSortFilterProxyModel::setFilterPattern(const QString& filterPattern)
{
    if (m_filterPatternRaw == filterPattern)
        return;

    m_filterPatternRaw = filterPattern;
    rebuildFilterRegExp();
...
void QQmlSortFilterProxyModel::rebuildFilterRegExp()
{
    QString pattern = m_filterPatternRaw;
    switch (m_filterPatternSyntax) {
        case Wildcard:
        case WildcardUnix:
            pattern = QRegularExpression::wildcardToRegularExpression(m_filterPatternRaw);
            break;
        case FixedString:
            pattern = QRegularExpression::escape(m_filterPatternRaw);
            break;
        case RegExp:
        case RegExp2:
        case W3CXmlSchema11:
        default:
            break; // used as-is, Perl-compatible syntax
    }
